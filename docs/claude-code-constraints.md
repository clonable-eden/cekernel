...
- Under `--bg`, sessions inherit the **daemon's** environment, not the
  ...
  that spawn auto-starts the daemon; a daemon left over from a previous
  run serves **stale** env (#589) — observed in the wild as
  cross-session IPC writes under a stale `CEKERNEL_SESSION_ID` (#688).
  `claude_bg_spawn` therefore SCRUBS `CEKERNEL_*` before invoking
  `claude --bg` (#688): no daemon carries any run's `CEKERNEL_*`, and
  sessions resolve it exclusively via env.sh / `.cekernel-env` (#652).
  The prompt remains the authoritative channel: agent
...
- **The daemon's inherited environment is unspecified** (#589, ADR-0018
  ...
  on it — and `claude_bg_spawn` scrubs `CEKERNEL_*` from the spawn env
  (#688) so no daemon can serve a stale session ID to later runs.
  Session env is guaranteed by the spawner via files: Workers source the
  worktree's `.cekernel-env` per Bash call (normative), Orchestrators
  source `${CEKERNEL_IPC_DIR}/env.sh` generated by
  `spawn-orchestrator.sh` (#652), plus prompt-embedded values.
