...
The `register` command generates a wrapper script per schedule entry at `~/.claude/cekernel/runners/<id>.sh`. The OS scheduler invokes this wrapper, not `claude -p` directly. The wrapper handles lockfile acquisition, environment setup, command execution, and registry status updates.
...
LOCK_DIR="$HOME/.claude/cekernel/locks/<repo-hash>.lock"
if ! mkdir "$LOCK_DIR" 2>/dev/null; then
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] SKIP locked" >> ~/.claude/cekernel/logs/cron.log
  exit 0
fi
...
cd /path/to/repo && claude -p --max-budget-usd 5 --no-session-persistence \
  "/dispatch --env headless --label ready" >> ~/.claude/cekernel/logs/cron.log 2>&1
STATUS=$?
