...
When more issues are queued than `CEKERNEL_MAX_WORKERS` allows, the Orchestrator sorts the queue by nice value (ascending) before spawning. This is the Unix `nice` model: lower value = higher priority = spawned first.
