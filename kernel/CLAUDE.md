...
ツールの利用可能性は agent frontmatter の `tools` で定義する。`.claude/settings.json` ではなく frontmatter を使うことで、worktree ごとの設定が不要になる。自動承認は `spawn-worker.sh` が CLI フラグ `--allowedTools` で指定する。frontmatter の `permissionMode` は subagent（Task tool 経由）向けであり、`--agent`（CLI 起動）には CLI フラグが必要なため。スキルファイルでは `allowed-tools` を使用する（エージェントとスキルでキー名が異なる点に注意）。
