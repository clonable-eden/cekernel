...
権限は agent frontmatter の `tools` で定義する。`.claude/settings.json` ではなく frontmatter を使うことで、worktree ごとの設定が不要になる。Worker は `claude --agent kernel:worker` で起動され、`--agent` フラグにより `tools` が自動適用される。スキルファイルでは `allowed-tools` を使用する（エージェントとスキルでキー名が異なる点に注意）。
