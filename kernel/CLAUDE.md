...
権限は agent frontmatter の `tools` と `permissionMode` で定義する。`.claude/settings.json` ではなく frontmatter を使うことで、worktree ごとの設定が不要になる。`tools` はツールの利用可能性を、`permissionMode` は自動承認を制御する。Worker は `claude --agent kernel:worker` で起動され、`--agent` フラグにより `tools` と `permissionMode` が自動適用される。スキルファイルでは `allowed-tools` を使用する（エージェントとスキルでキー名が異なる点に注意）。
