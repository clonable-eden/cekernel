...
コード変更を伴う issue では、テストファーストで実装を進める。

##### サイクル

1. **RED** — 期待する振る舞いを記述したテストを書き、**失敗を確認**してコミット
2. **GREEN** — テストを通す**最小限の**実装を書き、**成功を確認**してコミット
3. **REFACTOR** — 重複除去・命名改善・構造整理を行い、テストが通ることを確認してコミット

大きな issue は複数の小さなサイクルに分割する。

##### テストの原則

- 実装の内部状態ではなく**振る舞い**をテストする
- テスト間で状態を共有しない（各テストが独立）
- 外部依存（API、DB 等）はモックする
- エッジケースを忘れない: null/空配列/境界値/エラーパス

##### 省略してよいケース

ドキュメントのみの変更、設定ファイルの変更等、テストが不要・不適切な場合は TDD を省略してよい。
