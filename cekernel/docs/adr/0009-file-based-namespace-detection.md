...
The presence of `cekernel/agents/` in the repository root is the detection signal. This directory exists only in the cekernel source repository (self-hosting). In any other repository using cekernel as a plugin, this directory does not exist.
...
```
.claude/agents/
├── orchestrator.md@ -> ../../cekernel/agents/orchestrator.md
├── probe.md@        -> ../../cekernel/agents/probe.md
└── worker.md@       -> ../../cekernel/agents/worker.md

.claude/skills/
├── orchctrl@       -> ../../cekernel/skills/orchctrl
├── orchestrate@    -> ../../cekernel/skills/orchestrate
├── probe@          -> ../../cekernel/skills/probe
└── unix-architect@ -> ../../cekernel/skills/unix-architect
```
...
In a self-hosting repository (where `cekernel/agents/` exists), users should invoke skills without the namespace prefix:
...
The probe skill (`cekernel/skills/probe/SKILL.md`) and probe agent (`cekernel/agents/probe.md`) were created to verify D2 file-based detection across 4 scenarios. Each scenario compared LLM-based detection, file-based detection (D2), and agent spawning:
...
- Plugin-mode repositories that happen to contain a `cekernel/agents/` directory (unlikely but possible) would be misdetected as local mode
