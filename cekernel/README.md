...
cekernel/
  ...
  skills/
    orchestrate/
      SKILL.md               # /cekernel:orchestrate スキル
  scripts/
    orchestrator/
  ...
  tests/
    ...
    orchestrator/test-*.sh   # Orchestrator スクリプトのテスト
...
export CEKERNEL_SESSION_ID=glimmer-7861a821 && cecekernel/scripts/orchestrator/worker-status.sh
export CEKERNEL_SESSION_ID=glimmer-7861a821 && cekernel/scripts/orchestrator/watch-workers.sh 4 5 6
...
`--agent` フラグによりエージェント定義の `tools` が適用される。

ツールの自動承認（パーミッション確認なし）は、対象リポジトリの `.claude/settings.json` に委譲する。
cekernel 側ではツール許可をハードコードしない。
...
cekernel を利用するリポジトリは、`.claude/settings.json` でツール許可を設定する必要がある。
Worker は worktree 内でこの設定ファイルを自動的に読み込み、パーミッション確認なしで動作する。
...
`allow` に Worker が使用するツールを列挙し、`deny` で危険なコマンドを明示的に拒否する。
リポジトリごとに許可するツールやコマンドを自由にカスタマイズできる。
...
cekernel が定義するのは**ライフサイクル**（spawn → PR → CI → merge → notify → cleanup）だけである。

Worker が実際にコードを書く際は、**対象リポジトリの CLAUDE.md とプロジェクト規約に完全に従う**。
cekernel のルールが対象リポジトリの規約と矛盾する場合、対象リポジトリが常に優先する。

```
cekernel の権限        対象リポジトリの権限
─────────────          ────────────────────
いつ PR を作るか        どう実装するか
...
```bash
cekernel/scripts/orchestrator/watch-logs.sh             # 全 Worker
cekernel/scripts/orchestrator/watch-logs.sh 4           # 特定 Worker
```
