#!/usr/bin/env bash
# test-terminal-adapter.sh — terminal-adapter.sh のテスト
#
# WezTerm コマンドをモックし、adapter 関数の振る舞いを検証する。
set -euo pipefail
...
# ── Test 2: terminal_available — wezterm がない場合 ──
unset -f wezterm 2>/dev/null || true
# command -v wezterm が失敗するよう PATH を空にする（一時的）
OLD_PATH="$PATH"
PATH=""
if terminal_available; then
...
# ── Test 3: terminal_resolve_workspace — WEZTERM_PANE 未設定 ──
...
# ── Test 4: terminal_resolve_workspace — 正常系 ──
...
# ── Test 5: terminal_resolve_workspace — pane が見つからない場合 ──
export WEZTERM_PANE=999
RESULT=$(terminal_resolve_workspace)
assert_eq "resolve_workspace: WEZTERM_PANE=999 returns empty" "" "$RESULT"

# ── Test 6: terminal_spawn_window — workspace 指定あり ──
MOCK_LOG=$(mktemp)
wezterm() {
  echo "$*" >> "$MOCK_LOG"
...
# ── Test 7: terminal_spawn_window — workspace 指定なし ──
MOCK_LOG=$(mktemp)
wezterm() {
  echo "$*" >> "$MOCK_LOG"
  if [[ "$1" == "cli" && "$2" == "spawn" ]]; then
...
# ── Test 8: terminal_run_command — テキスト送信 + Enter ──
MOCK_LOG=$(mktemp)
wezterm() {
...
# ── Test 12: terminal_kill_window — 同一ウィンドウの全ペイン kill ──
MOCK_LOG=$(mktemp)
wezterm() {
  echo "wezterm $*" >> "$MOCK_LOG"
  if [[ "$1" == "cli" && "$2" == "list" ]]; then
...
# window 2 の pane 20 は kill されていないことを確認
