...
# ── Test 3: backend_worker_alive — returns 0 for running process ──
...
# ── Test 5: backend_worker_alive — returns 1 for non-existent handle ──
...
# ── Test 6: backend_kill_worker — no error for non-existent handle ──
EXIT_CODE=0
