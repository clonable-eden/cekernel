#!/usr/bin/env bash
# test-process-status-state.sh — Tests for process-status.sh state field integration
set -euo pipefail
...
echo "test: process-status state integration"

export CEKERNEL_SESSION_ID="test-pstatus-state-00000001"
