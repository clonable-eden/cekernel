...
# ── Test 4: Guard triggers at MAX_WORKERS=3 with 3 active ──
# Inline verification of spawn-worker.sh guard logic
MAX_WORKERS=3
...
# ── Test 6: MAX_WORKERS=5 with 2 active → guard not triggered ──
MAX_WORKERS=5
ACTIVE=$(active_worker_count)
if [[ "$ACTIVE" -ge "$MAX_WORKERS" ]]; then
  GUARD_TRIGGERED="yes"
