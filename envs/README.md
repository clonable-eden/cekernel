...
| `CEKERNEL_GC_STALE_BLOCKED_GRACE` | `600` | Positive integer (seconds) | `orchctl.sh` (`gc`) | Grace period in the stale-blocked orchestrator triple guard (ADR-0018 Amendment 1): gc reaps a `stale-blocked` (phantom blocked) orchestrator session only when all child workers are TERMINATED AND the session's IPC dir has been quiescent for at least this long. Any guard failing keeps the session; genuine `blocked` is never reaped |
